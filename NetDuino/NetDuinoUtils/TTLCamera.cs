...
namespace Onoffswitch.NetDuinoUtils
{
    public class TTLCamera : IDisposable
    {
        ...
        public void TakePictureToSocket(Socket connection, bool mjpeg = false)
        {
            ...
            if(mjpeg)
            {
                connection.Send(WebUtils.CreateMPEGFrameHeader(frameLength));
            }
            ...
            while (frameLength > 0)
            {
                ...
                if (connection != null)
                {
                    try
                    {
                        connection.Send(_frameBuffer, 0, segmentLength, SocketFlags.None);
                    }
                    catch (Exception ex)
                    {
                        break;
            ...
            if(mjpeg)
            {
                if (connection != null)
                {
                    try
                    {
                        connection.Send(WebUtils.CreateFooter());
                    }
                    catch (Exception ex)
                    {
                    }
            ...
            Debug.Print("Total bytes read: " + totalBytesRead);
            Debug.Print("Took " + (DateTime.Now - now));
        ...
        public void TakePictureToPath(string path)
        {
            using (var file = new FileStream(path, FileMode.Create, FileAccess.Write))
            {
                ...
                while (frameLength > 0)
                {
                    var segmentLength = System.Math.Min(frameLength, _frameBuffer.Length);
                    totalBytesRead += ReadFrameSegment(segmentLength, _frameBuffer, frameAddress);
                    file.Write(_frameBuffer, 0, segmentLength);
                    frameLength -= segmentLength;
                    frameAddress += segmentLength;
                }
